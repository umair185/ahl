...
<div class="page-body display" id="printableArea">
    <div class="row">
        <div class="col-md-12">
            <button class="btn btn-danger waves-effect waves-light" id='cancel_br'>Bulk Cancel</button>
            <div class="card">
                        ...
                        <table class="table table-hover" id="example">
                            ...
                            <tbody id="parcel-data">
                                ...
                                @foreach($orders as $order)
                                    <tr id="tr-{{$order->id}}">
                                        <td align="center"><input type="checkbox" class="case" name="case" value="{{$order->id}}"/></td>
    ...
    $("#cancel_br").click(function () {
        ...
        if(proceed)
        {
            ...
            if (checkedArray.length == 0) {
                alert('please select some Parcels for Bulk Cancel');
            } else {
                //var newWin = window.open();
                var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
                $.ajax({
                    /* the route pointing to the post function */
                    url: '/bulk-cancel-by-vendor',
                    type: 'POST',
                    /* send the csrf-token and the input to the controller */
                    data: {_token: CSRF_TOKEN, parcels: checkedArray},
                    dataType: 'json',
                    /* remind that 'data' is the response of the AjaxController */
                    success: function (response) {
                        if(response.status == 1){                            
                            alert('Parcels Cancelled, let the page refresh to view results');
                            location.reload();
                        }
                    }
                });
