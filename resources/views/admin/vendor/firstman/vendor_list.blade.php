...
<div class="page-body display" id="printableArea">
    <div class="card">
        <div class="card-header">
            <h5>Pickup Supervisor Vendors List</h5>
        </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    @foreach($vendor->pickupLocation as $key => $location)
                                        {{++$key}} - {{$location->address}}<br>
                                    @endforeach
                                </td>
                                @if($vendor->pickup_datentime == NULL)
                                <td></td>
                                @else
                                <td>{{date('d M Y h:i a', strtoTime($vendor->pickup_datentime))}}</td>
                                @endif
                                <td>{{$vendor->pickupAssignedBy ? $vendor->pickupAssignedBy->name : ''}}</td>
